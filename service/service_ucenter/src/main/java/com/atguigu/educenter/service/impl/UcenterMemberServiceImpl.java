...
public class UcenterMemberServiceImpl extends ServiceImpl<UcenterMemberMapper, UcenterMember> implements UcenterMemberService {
    ...
    public String panDuanuCenterMember(UcenterMember ucenterMember) {
        ...
        ucenterMember1 = baseMapper.selectOne(queryWrapper);
        if(ucenterMember1==null){
            throw new CustomException(20001,"手机号不存在,登录失败");
        }
        //判断密码是否正确,把输入的密码经MD5加密后在于数据库中的密码比较
        System.out.println(ucenterMember1.getPassword());
        System.out.println(MD5.encrypt(password));
        if(!ucenterMember1.getPassword().equals(MD5.encrypt(password))){
            throw new CustomException(20001,"密码错误,登录失败");
        }
        //判断用户是否禁用
        if(ucenterMember1.getIsDisabled()){
            throw new CustomException(20001,"用户禁用,登录失败");
        }
        //根据JWT规则生成token字符串
